...
from data.tokenize import SOS_idx, batch2TrainData
...
def run_experiment(model_name, src_voc, tar_voc, encoder, decoder,
    ...
    training_batches = [batch2TrainData(src_voc, tar_voc, [random.choice(train_set_pairs) for _ in range(batch_size)])
                      for _ in range(n_iteration)]

    # Load batches for each iteration
    val_batches = [batch2TrainData(src_voc, tar_voc, [random.choice(val_set_pairs) for _ in range(batch_size)])
                   for _ in range(n_iteration)]
